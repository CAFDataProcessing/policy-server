...
/**
 * Example extension of Condition class for use in testing.
 */
public class TestCondition extends Condition {

    @Autowired
    public TestCondition(EnvironmentSnapshot environmentSnapshot) {
        this.id = (Helper.getId());
        when(environmentSnapshot.getCondition(this.id)).thenReturn(this);
    }
}
